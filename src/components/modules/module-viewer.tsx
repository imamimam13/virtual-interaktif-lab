...
import { ArrowLeft, Menu, ChevronLeft, ChevronRight, CheckCircle, Circle } from "lucide-react";
        ...
        <div className="flex h-[calc(100vh-4rem)] flex-col md:flex-row bg-gray-50 dark:bg-zinc-900">
            ...
            <aside className="hidden w-80 border-r bg-white dark:bg-zinc-950 md:flex flex-col">
                <div className="p-4 border-b flex items-center justify-between">
                    ...
                    <Link href={`/dashboard/labs/${labId}`}>
                        <Button variant="ghost" size="sm"><ArrowLeft className="h-4 w-4" /></Button>
                    </Link>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {allModules.map((m, idx) => (
                        <Link key={m.id} href={`/dashboard/module/${m.id}`}>
                            <div className={cn(
                        ...
                        <SheetContent side="left">
                            ...
                            <div className="py-4">
                                ...
                                <div className="space-y-2">
                                    {allModules.map((m, idx) => (
                                        <Link key={m.id} href={`/dashboard/module/${m.id}`} onClick={() => setIsSidebarOpen(false)}>
                                            <div className={cn(
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col">
                ...
                <div className="mt-8 pt-4 border-t flex justify-between max-w-5xl mx-auto w-full">
                    {prevModule ? (
                        <Link href={`/dashboard/module/${prevModule.id}`}>
